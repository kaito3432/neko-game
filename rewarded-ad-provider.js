(function(factory){
  ...
  function createProvider({plugin=nativePlugin(),api=null,config={},onState=()=>{},sleep=ms=>new Promise(resolve=>setTimeout(resolve,ms))}={}){
    let initialized=false,busy=false,state=plugin?STATES.INITIALIZING:STATES.UNAVAILABLE;
    ...
    const emit=(next,detail={})=>{state=next;onState({state,...detail});};
    ...
    async function showRewardedAd(){
      ...
      busy=true;let handles=[],rewarded=false,dismissed=false;
      try{
        ...
        const attempt=await api.createAttempt(REWARD_TYPE);
        emit(STATES.LOADING);
        ...
        for(let index=0;index<attempts;index++){
          try{completed=await api.completeAttempt({rewardType:REWARD_TYPE,verificationId:attempt.attemptId,verification:{provider:'admob-ssv'}});break;}
        ...
        emit(STATES.COMPLETED,{verified:true,profile:completed.profile});
      ...
      }finally{await removeAll(handles);busy=false;}
    }
  ...
  function browserApi(){
    return {
      ...
      async completeAttempt(payload){
        ...
        root.dispatchEvent?.(new root.CustomEvent('nyan-rewarded-ad-progress',{detail:value}));return value;
      }
    };
  }
  ...
  return Object.freeze({REWARD_TYPE,IOS_TEST_REWARDED_ID,ANDROID_TEST_REWARDED_ID,STATES,createProvider,nativePlugin,provider});
});
